...
// Fetch movies based on search term for the logged-in user
...
if ($search_term) {
    $sql .= " AND (LOWER(Title) LIKE LOWER(?) OR LOWER(Genre) LIKE LOWER(?))";
    $params[] = "%$search_term%";
    $params[] = "%$search_term%";
    $types .= "ss";
}
...
<head>
  ...
  <title>RELAX MOVIE - Search</title>
...
<body class="text-white min-h-screen flex flex-col">
  ...
  <nav class="glass sticky top-0 z-50 text-white p-4 shadow-md flex justify-between items-center backdrop-blur sm:flex-row flex-col">
    ...
    <div class="flex items-center space-x-3 text-sm sm:mt-0 mt-3">
      ...
      <a href="search.php" class="hover:scale-105 transition bg-red-600 px-4 py-2 rounded-lg hover:bg-red-500">Search</a>
  ...
  <main class="p-6 sm:p-8 max-w-6xl mx-auto w-full flex-grow">
    <h2 class="text-4xl font-bold mb-8 text-center">🔍 Search Movies</h2>

    <!-- Search Form -->
    <div class="mb-6 flex justify-center">
      <form action="search.php" method="GET" class="w-full max-w-lg">
        <label for="searchInput" class="sr-only">Search movies by title or genre</label>
        <input type="text" id="searchInput" name="q" class="search-input" placeholder="Search movies by title or genre..." value="<?= htmlspecialchars($search_term) ?>" autocomplete="off">
      </form>
    </div>
  ...
  <script>
    ...
    function renderMovies(searchTerm = "") {
      ...
      if (filtered.length === 0) {
        movieGrid.innerHTML = '<p class="text-center text-gray-300">No movies found.</p>';
        return;
      }
    ...
    // Initial Load
    renderMovies("<?php echo addslashes($search_term); ?>");
